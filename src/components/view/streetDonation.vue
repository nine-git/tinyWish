      ...
      api.getFormsResponsesAPI(this.formId).then(res => {
        ...
        res.data.forEach((item)=> {
          ...
          if (item.mapped_values.review_street_state){
            objData.audit.statusStreet = item.mapped_values.review_street_state.value[0].value
            objData.audit.option_idStreet = item.mapped_values.review_street_state.value[0].id
          }else{
          ...
          if (item.mapped_values.street_retrun_reject){
            objData.pepole.retrunReject = item.mapped_values.street_retrun_reject.value[0]
          }
        ...
        console.log(this.unpassFormData)
      })
    },
    methods: {
      //通过的函数
