    ...
    <div class="streetWish">
      ...
      <van-popup v-model="show"
        ...
        <div class="contentOverlay">
          ...
          <van-field v-if="formData.audit.statusStreet==='已退回'" label="退回原因：" readonly type="text" v-model="formData.pepole.rejectdesc" />
          <van-field v-if="formData.audit.statusStreet==='待审核'" autosize label="回退理由："  type="textarea" v-model="myTextArea" />
          <p class="textNum" v-if="myTextArea.length<=maxtext&&formData.audit.statusStreet==='待审核'">{{myTextArea.length||0}}/{{maxtext}}</p>
          <p class="textNum" v-else-if="myTextArea.length>maxtext&&formData.audit.statusStreet==='待审核'"><span :style="{ color:'red' }">{{myTextArea.length||0}}</span>/{{maxtext}}</p>
      ...
      api.getFormsResponsesAPI('328').then((res) => {
        ...
        res.data.forEach((item)=> {
          ...
          if (item.mapped_values.streetAuditStatus){
            objData.audit.statusStreet = item.mapped_values.streetAuditStatus.value[0].value
            objData.audit.option_idStreet = item.mapped_values.streetAuditStatus.value[0].id
          }else{
          ...
          if (item.mapped_values.street_retrun_reject){
            objData.pepole.rejectdesc = item.mapped_values.street_retrun_reject.value[0]
          }
          ...
          if (objData.audit.statusStreet==="待审核"&&objData.audit.status==="已通过"){
            this.auditFormData.push(objData)
          } else if (objData.audit.statusStreet==="已通过"&&objData.audit.status==="已通过"){
            this.passFormData.push(objData)
          } else if (objData.audit.statusStreet==="已退回"&&objData.audit.status==="已通过"){
