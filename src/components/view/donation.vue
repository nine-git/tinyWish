    ...
    >
      ...
      <div class="contentOverlay">
        ...
        <p class="textNum" v-else-if="myTextArea.length>maxtext&&formData.audit.status==='待审核'">
          <span :style="{ color:'red' }">{{myTextArea.length||0}}</span>/200
        </p>
      ...
      res.data.forEach((item) => {
        ...
        if (item.mapped_values.retrunReject) {
          objData.pepole.retrunReject =
            item.mapped_values.retrunReject.value[0];
        }
...
>>>>>>> b2ee20a0e763d7b7682e94dba69990a2201476cf
          ...
          if (item.mapped_values.retrunReject){
            ...
            console.log(objData.pepole.retrunReject)
          }
          for (let y = 0; y < item.entries.length; y++) {
            //  对象的图片路径
            if (item.entries[y].attachment) {
...
=======
  ...
  methods: {
    ...
    getNewTime() {
      ...
      let year = newTime.getFullYear();
      let month =
        newTime.getMonth() + 1 > 9
          ? newTime.getMonth() + 1
          : "0" + (newTime.getMonth() + 1);
      let day = newTime.getDate();
      let hours = newTime.getHours();
      let minutes =
        newTime.getMinutes() + 1 > 9
          ? newTime.getMinutes() + 1
          : "0" + (newTime.getMinutes() + 1);
      return year + "-" + month + "-" + day + " " + hours + ":" + minutes;
    },
    //通过的函数
    pass() {
